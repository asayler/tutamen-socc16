...
\item[Certificate Authority Role:] Each access control server acts as
  ...
  each AC server must store its CA keys is a secure manner and
...
\begin{packed_desc}
\item[Token Verification:] Each storage server securely (via HTTPS)
  must obtain the public token signing key from each AC server
